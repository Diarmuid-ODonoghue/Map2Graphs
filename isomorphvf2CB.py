...
#    subgraph_isomorphisms_iter.__doc__ += "\n" + subgraph.replace('\n','\n'+indent)
    ...
    def syntactic_feasibility(self, G1_node, G2_node):
        ...
        #dod2020()
        # R_neighbor
        if isinstance(G1_node, str) and isinstance(G2_node, str) and second_word(G1_node) != second_word(G2_node):
            #print(".", end="")
            return False
...
#G1 = nx.path_graph(3)
#G1.add_edge(0,3)
G1.add_edges_from([("larry","curly"), ("curly","moe"), ("larry", "jim")])
#G1.add_edge(4,1)
...
class DiGraphMatcher(GraphMatcher):  # April 9 2020
    ...
    print(" VF2CB DiGraph ", end="")
    pass


class MultiGraphMatcher(GraphMatcher):
    """VF2 isomorphism checker for undirected multigraphs. """
    print(" VF2CB MultiGraph ", end="")
    pass


class MultiDiGraphMatcher(DiGraphMatcher):
    """VF2 isomorphism checker for directed multigraphs. """
    print("VVF2CB MultiDiGraph   ", end="")
    pass


edgelist1 = [(0, 1), (1,2), (2,3)]  # single edge (0,1)
edgelist2 = [(0, 1), (2,3)]  # single edge (0,1)
G1 = nx.from_edgelist(edgelist1)
G2 = nx.from_edgelist(edgelist2)
GM = MultiDiGraphMatcher(G2, G1)
GM.subgraph_is_isomorphic()  # invoke the mapping
print(GM.mapping)
...
#G2 = nx.from_edgelist([('a','b'), ('b','c')])
G2 = nx.from_edgelist([('a','b')])
GM = isomorphism.GraphMatcher(G1, G2)
#GM = isomorphvf2CB.MultiDiGraphMatcher(G1, G2)
GM = MultiDiGraphMatcher(G1, G2)
#GM = GraphMatcher(G1, G2)
print("Find smaller G2 inside bigger G1 - iso VF2CB Here", GM.subgraph_is_isomorphic())
print(GM.mapping)
print("########## Sensitive to the order of parameters. ")
GM = MultiDiGraphMatcher(G2, G1)
#GM = GraphMatcher(G1, G2)
print("iso VF2CB Here", GM.subgraph_is_isomorphic())
print(GM.mapping)